...
class ProxyHandler(object):
    ...
    def get(self, https=False, cn=False):
        """
        return a proxy
        Args:
            https: True/False
        Returns:
        """
        proxy = self.db.get(https, cn)
        return Proxy.createFromJson(proxy) if proxy else None

    def pop(self, https=False, cn=False):
        """
        return and delete a useful proxy
        :return:
        """
        proxy = self.db.pop(https, cn)
        if proxy:
    ...
    def getAll(self, https=False, cn=False):
        """
        get all proxy from pool as Proxy list
        :return:
        """
        proxies = self.db.getAll(https, cn)
